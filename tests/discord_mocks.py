"""
Mock Classes for Discord.py.

Adapted from python-discord/bot.
https://github.com/python-discord/bot/blob/main/tests/helpers.py

Note: Logging configuration has been moved to conftest.py pytest_configure hook.
"""
...
class CustomMockMixin:
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(
            spec_set=self.spec_set, **kwargs  # pyright: ignore[reportCallIssue]
        )

        if self.additional_spec_asyncs:
            self._spec_asyncs.extend(  # pyright: ignore[reportAttributeAccessIssue]
                self.additional_spec_asyncs
            )
    ...
    def _get_child_mock(self, **kw):
        ...
        if (
            ...
            and _new_name
            in unittest.mock._async_method_magics  # pyright: ignore[reportAttributeAccessIssue]
        ):
        ...
        if self._mock_sealed:  # pyright: ignore[reportAttributeAccessIssue]
            attribute = "." + kw["name"] if "name" in kw else "()"
            mock_name = (
                self._extract_mock_name()  # pyright: ignore[reportAttributeAccessIssue]
                + attribute
            )
...
guild_instance = discord.Guild(data=guild_data, state=unittest.mock.MagicMock())  # type: ignore[arg-type]
...
role_instance = discord.Role(
    guild=guild_instance, state=unittest.mock.MagicMock(), data=role_data  # type: ignore[arg-type]
)
...
class MockRole(CustomMockMixin, unittest.mock.Mock, ColourMixin, HashableMixin):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        if isinstance(self.permissions, int):  # type: ignore[has-type]
            self.permissions = discord.Permissions(self.permissions)  # type: ignore[has-type]
...
member_instance = discord.Member(
    data=member_data, guild=guild_instance, state=state_mock  # type: ignore[arg-type]
)
...
text_channel_instance = discord.TextChannel(state=state, guild=guild, data=channel_data)  # type: ignore[arg-type]

channel_data["type"] = "VoiceChannel"
voice_channel_instance = discord.VoiceChannel(
    state=state, guild=guild, data=channel_data  # type: ignore[arg-type]
)
...
dm_channel_instance = discord.DMChannel(me=me, state=state, data=dm_channel_data)  # type: ignore[arg-type]
...
message_instance = discord.Message(state=state, channel=channel, data=message_data)  # type: ignore[arg-type]
...
class MockMessage(CustomMockMixin, unittest.mock.MagicMock):
    ...
    def __init__(self, **kwargs) -> None:
        default_kwargs: dict[str, list] = {"attachments": []}
        super().__init__(**collections.ChainMap(kwargs, default_kwargs))
